...
from ....structures import gamma_correct
...
def blender_value_to_string(blender_value):
    ...
    if isinstance(blender_value,
        ...
        if len(value_list) == 4:
            # vec4 is color initialization, use gamma correction
            for idx, val in enumerate(value_list[:3]):
                value_list[idx] = gamma_correct(val)

        return "vec%d(%s)" \
            % (len(value_list), ", ".join([str(x) for x in value_list]))
